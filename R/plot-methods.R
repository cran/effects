...
{  
    ...
    if (n.predictors == 1){
        ...
        ### variate, no other predictors  ***
            ...
            else c(min(min(residuals), range[1] - .025*(range[2] - range[1])), 
                   max(max(residuals), range[2] + .025*(range[2] - range[1])))
            ...
            panel=function(x, y, subscripts, x.vals, rug, lower, upper, has.se, ...){
                ...
                if (has.se){  
                    ...
                    if (!is.null(residuals)){
                        ...
                        n.in.panel <- sum(use)
                        if (n.in.panel > 0){
                            lpoints(trans(x.fit[use]), residuals[use], col=residuals.color, pch=residuals.pch, cex=residuals.cex)
